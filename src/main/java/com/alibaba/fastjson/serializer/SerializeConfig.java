...
public class SerializeConfig {
	...
	public ObjectSerializer getObjectWriter(Class<?> clazz, boolean create) {
        ...
        if ((writer = getSerializerFromAutowiredObject(clazz)) != null) {
            return writer;
        }
        if ((writer = createSerializerFromModule(clazz)) != null) {
            return writer;
        }
        if ((writer = getSpecificClassTypeSerializer(clazz, create)) != null) {
            return writer;
        }
        return null;
    }
    ...
    /**
     * 读取clazz对应类型的AutowiredObjectSerializer
     * @param clazz
     * @return
     */
    private ObjectSerializer getSerializerFromAutowiredObject(Class<?> clazz) {
        ObjectSerializer writer = getAutowiredWriterFromSpecifyClassLoader(Thread.currentThread().getContextClassLoader(), clazz);
        if (writer != null) {
            return writer;
        }
        if (Thread.currentThread().getContextClassLoader() == JSON.class.getClassLoader()) {
          return null;
        }
        //如果当前线程的类加载器和JSON的类加载不是一个，尝试从JSON的类加载中加载
        writer = getAutowiredWriterFromSpecifyClassLoader(JSON.class.getClassLoader(), clazz);
        if (writer != null) {
            return writer;
        }
        return null;
    ...
    /**
     * 从classLoader中加载可见的AutowiredObjectSerializer
     * @param classLoader
     * @param clazz
     * @return
     */
    private ObjectSerializer getAutowiredWriterFromSpecifyClassLoader(final ClassLoader classLoader, Class<?> clazz) {
        try {
    ...
    /**
     * 基于modules创建ObjectSerializer
     * @param clazz
     * @return
     */
    private ObjectSerializer createSerializerFromModule(Class<?> clazz) {
        ObjectSerializer writer = null;
    ...
    /**
     * 获取Enum类型的ObjectSerializer
     * @param clazz
     * @return
     */
    private ObjectSerializer getEnumSerializer(Class<?> clazz) {
        ObjectSerializer writer = null;
    ...
    /**
     * 获取特定的awt序列化器，如果获取失败后续则会跳过
     * @param clazz
     * @return
     */
    private ObjectSerializer getSpecificAwtTypeSerializer(Class<?> clazz) {
        ObjectSerializer writer = null;
        String className = clazz.getName();
    ...
    /**
     * 获取一些特定的jdk8自带类型的序列化器，如果获取失败后续则会跳过
     * @param clazz
     * @return
     */
    private ObjectSerializer getSpecificJdk8TypeSerializer(Class<?> clazz) {
        ObjectSerializer writer = null;
        String className = clazz.getName();
        // jdk8
        if ((!jdk8Error) //
    ...
    /**
     * 获取一些特定oracle类型的序列化器，如果获取失败后续则会跳过
     * @param clazz
     * @return
     */
    private ObjectSerializer getSpecficOracleTypeSerializer(Class<?> clazz) {
        ObjectSerializer writer = null;
        String className = clazz.getName();
        if ((!oracleJdbcError) //
    ...
    /**
     * 获取一些特定类型的SpringFox的序列化器，如果获取失败，后续则跳过
     * @param clazz
     * @return
     */
    private ObjectSerializer getSpecificSpringFoxTypeSerializer(Class<?> clazz) {
        ObjectSerializer writer = null;
        String className = clazz.getName();
        if ((!springfoxError) //
    ...
    /**
     * 获取一些特定Guava类型的序列化器，如果获取失败，后续则会跳过
     * @param clazz
     * @return
     */
    private ObjectSerializer getSpecificGuavaTypeSerializer(Class<?> clazz) {
        ObjectSerializer writer = null;
        String className = clazz.getName();
        if ((!guavaError) //
    ...
    /**
     * 获取一些特定Joda类型的序列化器，如果获取失败，后续则会跳过
     * @param clazz
     * @return
     */
    private ObjectSerializer getSpecificJodaTypeSerializer(Class<?> clazz) {
        ObjectSerializer writer = null;
        String className = clazz.getName();
     ...
     * @return
     */
    private ObjectSerializer getOtherSpecificTypeSerializer(Class<?> clazz) {
        ObjectSerializer writer = null;
        String className = clazz.getName();
     ...
     * @return
     */
    private ObjectSerializer getAnnotationSerializer(Class<?> clazz) {
     ...
     * @return
     */
    private ObjectSerializer getProxySerializer(Class<?> clazz) {
        Class[] interfaces = clazz.getInterfaces();
     ...
     * @return
     */
    private ObjectSerializer getSpecificClassTypeSerializer(Class<?> clazz, boolean create) {
        ObjectSerializer writer = null;
        Class<?> superClass;
        ...
        if (Map.class.isAssignableFrom(clazz)) {
            ...
            writer = getEnumSerializer(clazz);
            ...
            if ((writer = getSpecificAwtTypeSerializer(clazz)) != null) {
                return writer;
            }
            if ((writer = getSpecificJdk8TypeSerializer(clazz)) != null) {
                return writer;
            }
            if ((writer = getSpecficOracleTypeSerializer(clazz)) != null) {
                return writer;
            }
            if ((writer = getSpecificSpringFoxTypeSerializer(clazz)) != null) {
                return writer;
            }
            if ((writer = getSpecificGuavaTypeSerializer(clazz)) != null) {
                return writer;
            }
            if ((writer = getSpecificJodaTypeSerializer(clazz)) != null) {
                return writer;
            }
            if ((writer = getOtherSpecificTypeSerializer(clazz)) != null) {
                return writer;
            }
            if ((writer = getAnnotationSerializer(clazz)) != null) {
                return writer;
            }
            if ((writer = getProxySerializer(clazz)) != null) {
                return writer;
            }
