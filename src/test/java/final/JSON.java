...
public abstract class JSON implements JSONStreamAware, JSONAware {
    ...
    public static <T> T parseObject(String text, Class<T> clazz) {
        ...
        for (Field field: fields){
            ...
            String s = new String();
            try{
                s = field.getAnnotation(JSONField.class).format();
                JSONObject jb = JSONObject.parseObject(text);
                jsonString = jb.getString(field.getName());
            }catch (Exception e){
                continue;
            }
            if (jsonString.length()!=0 && jsonString.length() != s.length())
    ...
    public final static String VERSION = "1.2.68";
}
